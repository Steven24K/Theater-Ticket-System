
        import * as React from "react"
import { NavLink } from "react-router-dom";
import { AppState, IAppState } from "../../AppState";
import FormMaster from "../../FormBuilder/components/FormMaster";
import { Func } from "../../FormBuilder/utils/Func";
import { omitOne } from "../../FormBuilder/utils/Omit";
import { actor } from "../../models/actor";
import { loadedAsyncState, loadingAsyncState, unloadedAsyncState } from "../../utils";
import { setCurrent_actor } from "../AdminFrontend/AdminData";
import { AsyncLoader } from "../shared/AsyncLoader";
import { Defaultactor } from "./Defaultactor";
import { createactor, getactorById, updateactor } from "./actor.api";

interface actorFormProps extends IAppState {

}

const setFormData = (mode: 'edit' | 'create', key: string, value: any) => (s1: AppState): AppState => {
    if (s1.page.kind != 'admin') return s1
    if (s1.page.entityData.kind != 'actor') return s1
    if (mode == 'edit') {
        if (s1.page.entityData.currentactor.data.kind != 'loaded') return s1
        return {
            ...s1, page: {
                ...s1.page, entityData: {
                    ...s1.page.entityData, currentactor: { kind: 'editing', data: loadedAsyncState({ ...s1.page.entityData.currentactor.data.value, [key]: value }) }
                }
            }
        }
    }
    return {
        ...s1, page: {
            ...s1.page, entityData: {
                ...s1.page.entityData,
                actorForm: { kind: 'editing', data: { ...s1.page.entityData.actorForm.data, [key]: value } }
            }
        }
    }
}

const submitForm = async (props: actorFormProps) => {
    if (props.appState.page.kind == 'admin' && props.appState.page.entityData.kind == 'actor') {
        if (props.route.match.params.action == 'edit') {
            if (props.appState.page.entityData.currentactor.data.kind == 'loaded') {
                props.setState(s => {
                    if (s.page.kind != 'admin') return s
                    if (s.page.entityData.kind != 'actor') return s
                    return ({
                        ...s, page: {
                            ...s.page, entityData: {
                                ...s.page.entityData,
                                currentactor: { ...s.page.entityData.currentactor, kind: 'updating' }
                            }
                        }
                    })
                })

                let res = await updateactor(props.appState.page.entityData.currentactor.data.value)

                props.setState(s => {
                    if (s.page.kind != 'admin') return s
                    if (s.page.entityData.kind != 'actor') return s
                    return ({
                        ...s, page: {
                            ...s.page, entityData: {
                                ...s.page.entityData,
                                actors: unloadedAsyncState(),
                                currentactor: { ...s.page.entityData.currentactor, kind: res == 200 ? 'updated' : 'update-failed' }
                            }
                        }
                    })
                })
            }
        } else if (props.route.match.params.action == 'create') {
            props.setState(s => {
                if (s.page.kind != 'admin') return s
                if (s.page.entityData.kind != 'actor') return s
                return ({
                    ...s, page: {
                        ...s.page, entityData: {
                            ...s.page.entityData,
                            actorForm: { ...s.page.entityData.actorForm, kind: 'submitting' }
                        }
                    }
                })
            })

            let res = await createactor(omitOne(props.appState.page.entityData.actorForm.data, 'Id'))

            props.setState(s => {
                if (s.page.kind != 'admin') return s
                if (s.page.entityData.kind != 'actor') return s
                return ({
                    ...s, page: {
                        ...s.page, entityData: {
                            ...s.page.entityData,
                            actorForm: {
                                data: res > 0 ? Defaultactor() : s.page.entityData.actorForm.data,
                                kind: res > 0 ? 'submitted' : 'submit-error'
                            }
                        },
                        actors: unloadedAsyncState()
                    }
                })
            })
        }
    }
}

export const Formactor = (props: actorFormProps) => {
    if (props.appState.page.kind != 'admin') return <div></div>
    if (props.appState.page.entityData.kind != 'actor') return <div></div>

    if ((props.appState.page.entityData.currentactor.data.kind == 'unloaded') && props.route.match.params.action == 'edit' && props.route.match.params.id && !isNaN(Number(props.route.match.params.id))) {
        props.setState(setCurrent_actor(loadingAsyncState(() => getactorById(Number(props.route.match.params.id)))))
    }

    let current_permission = props.appState.page.sidePanelState.get('actor')

    return <div>
        {
            props.appState.page.entityData.actorForm.kind == 'submitted' && <div className="alert alert-success" role="alert">
                New {props.appState.page.entityData.kind} added succesfully!
            </div>
        }

        {
            props.appState.page.entityData.currentactor.kind == 'updated' && <div className="alert alert-success" role="alert">
                {props.appState.page.entityData.kind} has updated succesfully!
        </div>
        }

        {
            (props.appState.page.entityData.currentactor.kind == 'update-failed' || props.appState.page.entityData.actorForm.kind == 'submit-error') && <div className="alert alert-warning" role="alert">
                An error occured while submitting form data, nothing is saved!
          </div>
        }

        {(
            (current_permission && current_permission.can_create && props.route.match.params.action == 'create') 
        || (current_permission && current_permission.can_edit && props.route.match.params.action == 'edit')
        )
         && <button className="btn btn-primary"
            onClick={() => submitForm(props)}
            disabled={props.route.match.params.action == 'create' && (props.appState.page.entityData.actorForm.kind == 'unsubmitted' ||
                props.appState.page.entityData.actorForm.kind == 'submitted' ||
                props.appState.page.entityData.actorForm.kind == 'submitting')
            }
        >
            {
                props.route.match.params.action == 'edit' ?
                    "Save edits" : "Create actor"
            }
        </button>}
        {
            current_permission && current_permission.can_view && props.route.match.params.action == 'edit' && props.route.match.params.id &&
            <NavLink className="btn btn-info" to={"/admin/" + props.appState.page.entityData.kind + "/view/" + props.route.match.params.id}>View</NavLink>
        }

        {props.appState.page.entityData.actorForm.kind == 'submitting' && <div>Submitting data...</div>}

        {props.route.match.params.action == 'create' && <h1>Create a new {props.appState.page.entityData.kind}</h1>}

        {props.route.match.params.action == 'edit' && props.appState.page.entityData.currentactor.data.kind == 'loaded' && <h1>Currently editing {props.appState.page.entityData.kind}: {props.appState.page.entityData.currentactor.data.value.Id}</h1>}


        {
            current_permission && current_permission.can_create && props.route.match.params.action == 'create' ?
                <FormMaster<actor>
                    id_prefix="actor_form"
                    defaultData={[props.appState.page.entityData.actorForm.data]}
                    onChange={(key, newValue, index) => props.setState(setFormData('create', key, newValue))}
                    query={Func(q => q.Select( 'Name',  'Password',  'DateOfBirth',  'Bio', ))}
                />
                :
                current_permission && current_permission.can_edit ?
                <AsyncLoader<actor> async={props.appState.page.entityData.currentactor.data}
                    onLoad={res => props.setState(s => {
                        if (s.page.kind != 'admin') return s
                        if (s.page.entityData.kind != 'actor') return s
                        return ({
                            ...s, page: {
                                ...s.page, entityData: {
                                    ...s.page.entityData,
                                    currentactor: { ...s.page.entityData.currentactor, data: res }
                                }
                            }
                        })
                    })}
                >
                    {props.appState.page.entityData.currentactor.data.kind == 'loaded' &&
                        <FormMaster<actor>
                            id_prefix="actor_form"
                            defaultData={[props.appState.page.entityData.currentactor.data.value]}
                            onChange={(key, newValue, index) => props.setState(setFormData('edit', key, newValue))}
                            query={Func(q => q.Select( 'Name',  'Password',  'DateOfBirth',  'Bio', ))}
                        />}

                </AsyncLoader>
                :
                <div>Permission denied</div>
        }

    </div>
}
        